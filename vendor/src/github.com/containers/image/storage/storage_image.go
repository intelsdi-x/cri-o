...
func newImageDestination(imageRef storageReference) (*storageImageDestination, error) {
	// If we're going to want to set a name, figure out the name.
	tag := ""
	dref := imageRef.DockerReference()
	if dref != nil {
		tag = verboseName(dref)
	}
...
func (s storageImageDestination) ShouldCompressLayers() bool {
	...
	// applying them, so callers shouldn't bother doing that.
	return true
}
...
func (s *storageImageDestination) PutBlob(stream io.Reader, blobinfo types.BlobInfo) (types.BlobInfo, error) {
	...
	if (n > 0) && archive.IsArchive(header[:n]) {
		...
		id := ddigest.Digest(blobinfo.Digest).Hex()
		...
		if err != nil && err != storage.ErrDuplicateID {
			logrus.Debugf("error importing layer blob %q: %v", blobinfo.Digest, err)
			return errorBlobInfo, err
		}
		if err == storage.ErrDuplicateID {
			...
			if err != nil && err != io.EOF {
				logrus.Debugf("error digesting layer blob %q: %v", blobinfo.Digest, err)
				return errorBlobInfo, err
			}
			hash = hasher.Sum(nil)
		} else {
		...
		// matches the ID. */
		if strings.HasPrefix(blobinfo.Digest, "sha256:") && blobinfo.Digest != "sha256:"+hex.EncodeToString(hash[:]) {
			logrus.Debugf("blob %q digests to %q, rejecting", blobinfo.Digest, hex.EncodeToString(hash[:]))
			if layer != nil {
				// Something's wrong; delete the newly-created layer.
				s.imageRef.transport.store.DeleteLayer(layer.ID)
			}
		...
		if layer != nil {
			logrus.Debugf("blob %q imported as a filesystem layer %q", blobinfo.Digest, digest)
		} else {
			logrus.Debugf("layer blob %q already present", blobinfo.Digest)
		}
	} else {
...
func (s *storageImageDestination) Commit() error {
	...
	for k, v := range s.BlobData {
		if err := s.imageRef.transport.store.SetImageBigData(img.ID, k, v); err != nil {
			if _, err2 := s.imageRef.transport.store.DeleteImage(img.ID, true); err2 != nil {
				logrus.Debugf("error deleting incomplete image %q: %v", img.ID, err2)
			}
...
func (s *storageImageSource) GetBlob(digest string) (rc io.ReadCloser, n int64, err error) {
	if _, ok := s.Layers[digest]; !ok {
		b, err := s.imageRef.transport.store.GetImageBigData(s.ID, digest)
		if err != nil {
			return nil, -1, err
		}
		r := bytes.NewReader(b)
	...
	return s.diffLayer(s.Layers[digest])
}

func (s *storageImageSource) diffLayer(layerID string) (rc io.ReadCloser, n int64, err error) {
...
func (s *storageImageSource) GetTargetManifest(digest string) (manifestBlob []byte, MIMEType string, err error) {
